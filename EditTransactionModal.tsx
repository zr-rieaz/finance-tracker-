...
export const EditTransactionModal: React.FC<EditTransactionModalProps> = ({
  ...
  // Sychronize form state with selected transaction details when modal opens
  useEffect(() => {
    if (transaction) {
      setTitle(transaction.title);
      // Convert stored USD rate to display (local) unit inside the input box form
      const displayAmt = displayBalance(transaction.amount, currency);
      setAmount(displayAmt.toFixed(2));
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    // Capture the month name correctly based on modified date picker
    ...
    onSubmit({
      ...transaction,
      title: title.trim(),
      amount: value, // Send the local currency value. App.tsx will convert it to internal USD correctly.
      category,
      type,
      date,
      month: monthName,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
      ...
      >
        {/* Header decoration */}
        <div className="bg-indigo-650 dark:bg-zinc-900 p-5 text-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center">
              <Save size={16} className="text-white" />
            </div>
            <span className="font-bold font-sans">
              {currentLang === 'bn' ? 'লেনদেন সংশোধন করুন' : 'Edit Transaction'}
            </span>
          </div>
          <button 
            type="button"
        ...
        {/* Modal Inputs Body */}
        <form onSubmit={handleFormSubmit} className="p-6 flex flex-col gap-4.5 bg-white dark:bg-zinc-955 text-gray-800 dark:text-zinc-250">
          {error && (
            <div className="bg-rose-50 dark:bg-rose-950/20 text-rose-650 dark:text-rose-400 p-3 rounded-2xl flex items-center gap-2 border border-rose-100 dark:border-rose-900/40 text-[11px] font-semibold">
              <AlertCircle size={15} />
              <span>{error}</span>
            </div>
          )}
          ...
          <div className={`grid gap-4 ${type === 'expense' ? 'grid-cols-2' : 'grid-cols-1'}`}>
            <div className="flex flex-col gap-1.5 font-sans animate-fadeIn">
          ...
          {/* Action Row */}
          <div className="flex gap-3 mt-3 font-sans">
            <button
              type="button"
              onClick={onClose}
            ...
            <button
              type="submit"
              className="flex-1 py-3 text-xs font-bold rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg active:scale-98 transition-all cursor-pointer"
            >
              {currentLang === 'bn' ? 'সংরক্ষণ করুন' : 'Save Changes'}
            </button>
          </div>
        </form>
      </div>
